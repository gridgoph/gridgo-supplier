...
- **Clerk identity** — `ClerkProvider` uses `@clerk/expo/token-cache` (SecureStore), while `components/ClerkSessionBridge.tsx` accepts only server-written `publicMetadata.gridgoRole === "supplier"` before hydrating `gridgo-api`'s `/auth/me` projection. Missing or mismatched access fails closed on `app/access.tsx`; never write role metadata client-side. The publishable key is baked in `app.config.ts` as `extra.clerkPublishableKey` from the build-time env and read from `Constants.expoConfig.extra` — not a raw `process.env` access at module scope. Release runtime still requires `pk_live_`; `__DEV__` may use `pk_test_`. **Public apply** is the create path: `POST /auth/signup` with `role: "supplier"` via `api.signupSupplier` / `session.signupSupplier`. That route works in `AUTH_MODE=legacy`; dual/Clerk answers `invitation_required` — do not flip `AUTH_MODE` and do not invent a Clerk create flow. Invitation accept (`__clerk_ticket`) stays a secondary path. Google may authenticate or create an identity, but never grants supplier access. A **development** build retains the replaceable local supplier fixture behind `__DEV__` in `lib/devLogin.ts`; Metro strips its credential and connection-line literals, proved by `app/__tests__/devLoginDisclosure.test.ts` and `scripts/assert-no-dev-credentials.mjs`.
...
- `app/(auth)/signup/` — public apply stepper (identity, location, services, documents, review). `lib/onboardingSteps.ts`, `lib/signup.ts`, and `store/signupDraft.ts` own the draft. After send, `verificationStatus: pending` is correct; `isMatchable` still keeps an unapproved shop on `app/accreditation.tsx`. Invitation accept remains at `app/(auth)/accept-invitation.tsx`.
...
- **Prebuild rewrites `package.json`, so commit before you run it.** It changes the `android`/`ios` scripts to `expo run:*`. `git checkout package.json` to undo that also throws away any *uncommitted* dependency — which is how `expo-notifications` was silently dropped from `dependencies` between install and build. Autolinking reads `dependencies`, not `node_modules`, so the package stayed on disk, `expo config --type public` still resolved, the APK still built, and it crashed on launch. Check `npx expo-modules-autolinking search -p android | grep expo-notifications` before blaming the plugin.
