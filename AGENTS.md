...
- **Custom auth** — email/password → bearer token; role enforced in Zustand session (`store/session.ts`). Mismatched role is rejected (no role switcher). Shops **sign themselves up** (`POST /auth/signup`); Operations no longer creates accounts. The pilot's demo accounts still exist, but **no credential belongs in this repo or on a screen**: their passwords come from the deployment's configuration, and the sign-in fields start empty because a hosted build shows whatever it prefills to anyone who opens the app (`app/__tests__/login.test.tsx`).
