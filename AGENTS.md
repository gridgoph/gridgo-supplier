...
- **Custom auth** — email/password → bearer token; role enforced in Zustand session (`store/session.ts`). Mismatched role is rejected (no role switcher). Shops **sign themselves up** (`POST /auth/signup`); Operations no longer creates accounts. The pilot's demo accounts still exist; production passwords come from deployment configuration. A **development** build may prefill the supplier fixture behind `__DEV__` in `lib/devLogin.ts` (Metro strips the branch, proved by `app/__tests__/devLoginDisclosure.test.ts` and `scripts/assert-no-dev-credentials.mjs`); a release build ships empty fields and no demo address or password in the bundle.
