...
- **Local Android** — a USB `expo-dev-client` build, not Expo Go. `npm start` is `expo start --dev-client`; `npm run android` is `expo run:android`. Metro for this app is port **8082**. Prebuild and `expo run:android` need `GOOGLE_SERVICES_JSON` pointing at the captain's Firebase file (never committed; see Push notifications). Do not restart a LAN Expo Go packager.
...
- `lib/alertsApi.ts` / `store/alerts.ts` — marking read, marking a named set read, and deleting all go to GRIDGO first. Those three routes and the live stream are **new and may not be deployed**, so a 404 is a third outcome (`not_open_yet`), not a failure: the decision is remembered on the device and the screen says plainly that it lives on this phone. **When they land, delete the `dismissed`/`deleted` lists and that caveat** — `lib/alertsApi`'s header names the whole cleanup. Mark-all sends the ids that are actually on screen, never "everything", or it would hide alerts that arrived while the shop was reading.
- `lib/alertStream.ts` + `lib/eventStream.ts` + `hooks/useAlertStream.ts` + `store/toasts.ts` — alerts arrive live over SSE (`XMLHttpRequest`, because RN's `fetch` cannot stream and there is no `EventSource`; no new dependency). It resumes on `Last-Event-ID`, reconnects when the app returns from the background, and **fails silently** — every screen already reloads on focus, and "the event stream is unavailable" is not something a shop can act on. A 404 stops it for the session rather than retrying a route this deployment does not have. Toasts sit at the **top**: every primary action in this app is at the foot of its screen. Nothing is toasted that the shop is already looking at (`shouldToast`).
...
- **The permission is asked once and a refusal is effectively permanent.** Nothing may call `requestPermissionsAsync` on launch; the only caller is a tap on `components/PushEnableCard.tsx`. That card is drawn in three places, each one a moment where being told something is the answer to a question the screen has already raised: the Alerts tab, a job **waiting on somebody else**, and the accreditation screen. Not on sign-in. Once blocked, `Linking.openSettings()` is the only honest offer.
...
- **A foreground push shows nothing** (`PUSH_FOREGROUND_BEHAVIOR`). This app announces an arrival twice already — the Alerts tab holds the record and `useAlertStream` toasts it on whatever screen the shop is on — so a banner would be a third. The arrival is spent recomputing the unread count from the list, which is idempotent and therefore safe to run alongside the stream's `+1`.
- **A tap opens the job's workspace** (`pushTargetRoute`), because every event worth interrupting a shop for is acted on there — Accept/Decline, the yellow proof step, the pickup handoff, the issue holding the money — and `lib/jobState` has already decided which leads. No job, or an unknown `type`: the Alerts tab. Both routes sit behind the **`matchable`** guard, so `usePushNotifications` defers a tap until the shop is approved rather than pushing into a route the guard would bounce.
- **The API does not address a single notification to a supplier yet.** `gridgo-api` creates notifications for the client (`supplier_assignment_final_price`), the rider (`pickup_escalation_resolved`) and Operations (`pickup_check_escalation`) — grep `store.notifications.push` in its `src/`. Everything on this side is live and proved on a device; the events a shop is *promised* (a job offered, proof owed before a payout releases, a rider arriving, a pickup check failing, a payout released, an issue raised) need the server half. Do not add a client-side workaround for that.
...
- **Prebuild rewrites `package.json`, so commit before you run it.** `android` is already `expo run:android`; prebuild still rewrites `ios` to `expo run:ios`. `git checkout package.json` to undo that also throws away any *uncommitted* dependency — which is how `expo-notifications` was silently dropped from `dependencies` between install and build. Autolinking reads `dependencies`, not `node_modules`, so the package stayed on disk, `expo config --type public` still resolved, the APK still built, and it crashed on launch. Check `npx expo-modules-autolinking search -p android | grep expo-notifications` before blaming the plugin.
