...
- **Session → routes** — `Stack.Protected` in `app/_layout.tsx` (SDK 54) guards Clerk restoring, signed out, access mismatch, signed in, and matchable states. Tabs mount for **any** signed-in supplier. A shop with `verificationStatus !== "approved"` still lands on `/(tabs)/home` (`lib/launch.ts`); Home tells the truth about the wait. Accreditation is a screen they open from Home / Settings / Account, not the post-auth trap. Job / payout / catalogue stay behind `isMatchable`. Unassigned Clerk (no GRIDGO membership) goes to apply, never `/access`. Do not sprinkle logout/401 redirects through domain screens; session state owns the guards.
