...
export default function ReviewStep() {
  ...
  const { signupSupplier, loading, error, clearError } = useSession();
  ...
  async function open() {
    ...
    const created = await signupSupplier(request);
    if (!created) return;
    ...
    <OnboardingStep
      ...
      footer={
        <>
          ...
          <PrimaryButton
            label={loading ? "Sending your application…" : "Send my application"}
            disabled={loading}
            onPress={() => void open()}
          />
        </>
